...
static int current_wpm_ocean_dream = 0;
...
static void animate_waves(void) {
    ...
    for (int i = 0; i < WIDTH; ++i) {
        ...
        if (is_calm || current_wpm_ocean_dream <= WAVE_CALM) {
            draw_ocean(0, offset, byte_index);
        } else if (current_wpm_ocean_dream <= WAVE_HEAVY_STORM) {
            draw_ocean(1, offset, byte_index);
        } else if (current_wpm_ocean_dream <= WAVE_HURRICANE) {
...
static void animate_island(void) {
    ...
    if (is_calm || current_wpm_ocean_dream < ISLAND_CALM) {
        draw_island_parts(0);
    } else if (current_wpm_ocean_dream >= ISLAND_CALM && current_wpm_ocean_dream < ISLAND_HEAVY_STORM) {
        draw_island_parts(island_frame_1 + 1);
    } else if (current_wpm_ocean_dream >= ISLAND_HEAVY_STORM && current_wpm_ocean_dream < ISLAND_HURRICANE) {
...
static void animate_shooting_stars(void) {
    ...
    int number_of_shooting_stars = current_wpm_ocean_dream / SHOOTING_STAR_WPM_INCREMENT;
...
void render_stars(void) {
    ...
    if (timer_elapsed32(starry_night_anim_timer) > STARRY_NIGHT_ANIM_FRAME_DURATION) {
        ...
        current_wpm_ocean_dream             = get_current_wpm();
